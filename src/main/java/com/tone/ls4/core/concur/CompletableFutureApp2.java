...
public class CompletableFutureApp2 {
     ...
     * @param discount 折扣
     */
    public boolean verifyOrderWithFuture(long userId, long itemId, double discount)
            throws ExecutionException, InterruptedException {
        //3 个 rpc 调用可以同时进行了，系统延迟降低为之前的 1/3。不过延迟降低吞吐量的问题还是没有解决，依然需要通过增加线程数来提升吞吐量。
        // 验证用户能否享受这一折扣，RPC调用
        Future<Boolean> verifyDiscountFuture = executor
                .submit(() -> discountVerify(userId, itemId, discount));
     ...
     * @param discount 折扣
     */
    public CompletableFuture<Boolean> verifyOrderCompletableFuture(long userId, long itemId,
            double discount) {

        // 验证用户能否享受这一折扣，RPC调用
        CompletableFuture<Boolean> verifyDiscountFuture = CompletableFuture
                .supplyAsync(() -> discountVerify(userId, itemId, discount));

        // 获取商品单价，RPC调用
        CompletableFuture<Double> itemPriceFuture = CompletableFuture
                .supplyAsync(() -> getPrice(itemId));

        // 获取用户账号余额，限定了只能使用余额购买，RPC调用
        CompletableFuture<Double> balanceFuture = CompletableFuture
                .supplyAsync(() -> getBalance(userId));
